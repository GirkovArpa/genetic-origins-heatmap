...
def main(self, input_filename, vendor):
    try:
        ...
        # self.call_function('yeet', 'Calculating ...')
        # get arguments
        # args = arguments()
        # admix -f my_raw_data.txt -v 23andme -m K36
        args = {
            'models': [ 'K36' ],
            'file': input_filename,
            'vendor': vendor,
        ...
        if (args['models'] is None):
            ...
            for m in models:
                if not m in all_models:
                    raise ValueError('Cannot find model ' + m + '!')
        ...
        if args['file'] == '':
            ...
            if args['vendor'] != '23andme':
                args['vendor'] = '23andme'
                print('Ignore the vendor argument (' + arg['vendor'] +
                    ') since demo only provided for 23andme.\n')

        # beginning of calculation
        print('Calcuation is started...\n')
        result = admix_results(models, args['output'], args['zhongwen'], args['tolerance'],
                    args['sort'], args['ignore_zeros'], args['vendor'], args['file'])

        self.call_function('yeet', result)
    except Exception as e:
        self.call_function('handle_python_error', e.__class__.__name__, str(e))
